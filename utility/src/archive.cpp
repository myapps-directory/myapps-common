        ...
        if (zip_stat_index(pzip, i, 0, &stat) == 0) {
            ...
            if (stat.name[name_len - 1] == '/') {
                ...
                if (pzf) {
                    ...
                    FileWriteFunctionT file_write_function = _create_file_writer_function(stat.name, stat.size);
...
bool archive_extract(const std::string& _path, const std::string& _root, uint64_t& _runcompressed_size)
{
    ...
    auto create_write_lambda = [&_root](const char* _file_name, uint64_t /*_size*/) {
